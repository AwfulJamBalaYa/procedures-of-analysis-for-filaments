pro chi_hist
	...
	good1 = where(gsquared gt 0.); and gsquared le 6) 
	bad1 = where(gsquared le 0.)
	good2 = where(psquared gt 0.); and psquared le 6)
	...
	cghistoplot, psquared[good2], binsize = 1, $
		xtitle = '!17 Reduced chi-squared values of Plummer-like fitting',$
			xrange = [0, 50], ytitle = '!17 Number', ytickformat = '(I)', /fill, $
				position = pos, datacolorname = ['black'],  POLYCOLOR=['blu5'], mininput = 0.
	cghistoplot, abs(psquared[bad2]), binsize = 1, datacolorname = ['red4'],  POLYCOLOR=['red4'], $
		/oplot, /line_fill, thick = 5, orientation = 45, /outline, mininput = 1.2
	cgps_close
	...
	pos = [0.2, 0.2, 0.7, 0.7]
	;pos = [0.15, 0.15, 0.9, 0.9]
	cghistoplot, gsquared[good1], binsize = 1, $
		xtitle = '!17 Reduced chi-squared values of Gaussian fitting',$
			xrange = [0, 50], ytitle = '!17 Number', ytickformat = '(I)', /fill, $
				position = pos, datacolorname = ['black'],  POLYCOLOR=['blu5'], mininput = 0.
	cghistoplot, abs(gsquared[bad1]), binsize = 1, datacolorname = ['red4'],  POLYCOLOR=['red4'], $
		/oplot, /line_fill, thick = 5, orientation = 45, /outline, mininput = 1.2
	cgps_close
end